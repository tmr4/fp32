; modified from
; from: http://65xx.unet.bz/fpu.txt
; or: https://web.archive.org/web/20170923012704/http://65xx.unet.bz:80/fpu.txt
; if above is gone
...
; Original copyright and Implementation notes:
;;
...
;;        ftrunc, fceil, floor, fround: rounding functions
;;        fpmod, fprem:          remainders functions
;;        fldu16, fldu32, fldu64, fldu128: convert unsigned integer to float
...
; see https://github.com/cc65/cc65/issues/881#issuecomment-744004841
;       for discussion on how to set up config file to have direct page
;       addressing with direct page other than page 0
.segment "DPFPU": direct

tm:     .res 16         ; temp. mantissa
fsubnf: .byte 0         ; subnormal flag used by fac2dec
sgncmp: .byte 0         ; sign comparison: fac vs. arg

; floating point accumulator (fac)
facm:   .res 12         ; significand (113 bits) + guard bits (15 bits)
facm32: .res  4         ; significand (24 bits) + guard bits (8 bits)
facexp: .word 0         ; biased exponent
facsgn: .byte 0         ; mantissa sign
facst:  .byte 0         ; status of floating point value
...
fexph:  .word 0         ; unbiased fac exponent sign extension
facext: .word 0         ; fac guard bits extension

; floating point operand (arg)
argm:   .res 12         ; significand (113 bits) + guard bits (15 bits)
argm32: .res  4         ; significand (24 bits) + guard bits (8 bits)
argexp: .word 0         ; biased exponent
argsgn: .byte 0         ; mantissa sign
argst:  .byte 0         ; status of floating point value
...
aexph:  .word 0         ; unbiased arg exponent sign extension
argext: .word 0

fcp:   .res 3           ; long pointer to flaot constants
scsgn: .byte 0          ; scaling sign
scexp: .word 0          ; scaling value
dexp: .word 0           ; decimal exponent
dsgn: .byte 0           ; decimal float sign
pdeg: .byte 0           ; polyn. degree

tlp: .res 3             ; string long pointer
fpidx: .byte 0          ; string index
...
fpprec: .word 0         ; precision
fpfmt: .byte 0          ; format
fpaltf: .byte 0         ; alternate format
fpcap: .byte 0          ; adding for lower case
fpstyle: .byte 0        ; flag 'F' style
...
P0FPU = $f000           ; direct page (arbitrary)
...
;MINGEXP = .loword(-4)   ; min. decimal exponent 'G' format
MINGEXP = <(-4)   ; min. decimal exponent 'G' format

;MAXBSHIFT = .loword(-MNTBITS + 1)        ; max. shift mant.
...
; code segment
; Entry -- all routines:
;       - registers set to 8-bit *** TODO: check how often we have an immediate shift to 16-bit ***
;       - direct page register set to P0FPU
;       - data bank register loaded with $00
;
;---------------------------------------------------------------------------
...
@shm:   acc8                              ; A=negative shift's count
...
@sh16:        reg16                        ; shift right 16 bit at time
        ldy        <2,x
        sty        <0,x
        ldy        <4,x
        sty        <2,x
...
shlmx:
;-----
.a16                ; should be called with A/M=16 bit
.i8
...
@sc:        jsr        mvf_t0                ; save fac (tfr0=fac) in temp. reg.
        ...
        asl        a
        tax
; *** wrong bank, this will be absolute index in Bank 0, our data is in Bank 1 ***
;	lda	>fcaddr+1,x
        lda        f:fcaddr+1,x
        tay
;	lda	>fcaddr,x
...
;        lda        #!fce8                ; now find the high order factor...
        lda        #.loword(fce8)                ; now find the high order factor...
;        lda        #a:fce8                ; now find the high order factor...
...
;------
        ...
        reg16                        ; clear the partial result
        stz        tm
        stz        tm+2
        stz        tm+4
        stz        tm+6
...
@done:        reg8
        rts
@z:        reg8                        ; underflow: load zero into fac...
        pla                        ; ...and exit
        pla
        jmp        fldz
...
;
; This routine rounds the 128-bit fac mantissa to 113-bits, packs to
; quadruple precision IEEE standard format, and stores it in memory
;
;-----
fpack:
;-----
...
@get0:  lda        [tlp],y              ; get char
        beq        @eos                 ; end of string
        cmp        #' '
; *** leading blanks have already been skipped by Forth interpreter 
;     but we don't have an asciiz string so check for blank to end ***
;        beq        @nx0                 ; skip leading blanks
        beq        @eos                 ; end of string
        cmp        #$0d
        beq        @eos                 ; end of string
...
@hex:        lda        facm,x
        ...
        sta        a:P0FPU+fpstr,y        ; store high digit
        iny
        xba
        sta        a:P0FPU+fpstr,y        ; store low digit
        iny
        dex
        bpl        @hex
        tyx
...
@a4:    lda        facm,x
        jsr        b2hex                ; convert
        sta        a:P0FPU+xcvt,y        ; store high digit
        iny
        xba
        sta        a:P0FPU+xcvt,y        ; store low digit
        iny
        dex
        bpl        @a4
...
@ffl:        lda        fpstr,x                ; regular digit
        beq        @ff2                ; end
        sta        a:P0FPU+xcvt,y        ; store digit
        iny
        inx
        dec        wftmp
        dec        scexp
...
@e0:        stx        wftmp                ; overall digits count
        ...
        lda        fpstr,x
        sta        a:P0FPU+xcvt,y        ; store first digit
        dec        wftmp
        inx
        iny
        lda        #'.'                ; decimal dot
...
@e2:        inx                        ; bump pointer
        ...
        iny
        xba                        ; 2nd digit
@e3:        sta        a:P0FPU+xcvt,y        ; store following digits
        iny
        dec        wftmp
...
@nf:        stx        fsubnf                ; remember if we prescaled by 1e64
        ...
        acc16
        inc        dexp                ; increment decimal exponent...
        acc8                        ; ...because next division by 10
...
@cvt1:        lda        facm                ; we use guard bits too in conversion
        ...
        sbc        #EBIAS+MNTBITS-1
        acc8                        ; negative or null, just 8 bits value
...
fcaddr:        
.word        .loword(fce0), .loword(fce1), .loword(fce2), .loword(fce3), .loword(fce4), .loword(fce5), .loword(fce6), .loword(fce7)
...
@ok:        acc16
        ...
        stz        fcpc1
        stz        fcpc2
        lda        #.loword(ce2p)                ; P coefficients
        sta        fcpp
        lda        #.loword(ce2q)                ; Q coefficients
        sta        fcpq
        ldx        #4                ; P: degree 4
        stx        fcpd
        stx        fcqd                ; Q: degree 4
...
@ok:        acc16
        lda        #.loword(lg2e)                ; log2(e)
        sta        fcpc0
        lda        #.loword(ln2c1)                ; loge(2) first piece
        sta        fcpc1
        lda        #.loword(ln2c2)                ; loge(2) 2nd piece
        sta        fcpc2
        lda        #.loword(ceep)                ; P coefficients
        sta        fcpp
        lda        #.loword(ceeq)                ; Q coefficients
        sta        fcpq
        ldx        #4                ; P: degree 4
        stx        fcpd
        inx
...
@ok:        acc16
        lda        #.loword(lg210)                ; log2(10)
        sta        fcpc0
        lda        #.loword(lg102a)                ; log10(2) first piece
        sta        fcpc1
        lda        #.loword(lg102b)                ; log10(2) 2nd piece
        sta        fcpc2
        lda        #.loword(ce10p)                ; P coefficients
        sta        fcpp
        lda        #.loword(ce10q)                ; Q coefficients
        sta        fcpq
        ldx        #5                ; degree
        stx        fcpd
        stx        fcqd
...
@z:        jmp        fldz                ; return 0
@ibig:        acc8                        ; y is a big integer and we check if... 
                                ; ...is odd or even
